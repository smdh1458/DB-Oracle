...
DOUBLE  : 실수형 자료형 
DECIMAL OR NUMBERIC : 고정 소수점을 표현한 숫자

문자자료형

고정 길이
CHAR(n) : 길이를 고정적으로 설정
         무조건 길이가 짧은 적든 초기에 작성한 길이만큼 공간을 설정

가변 길이
VARCHAR : 변수 공간의 길이를 변경가능한 문자열
          실제 데이터의 길이만큼 저장                           최대 225자(숫자, 영어 기준) 저장 가능  
TEXT       : 긴글자를 저장할 때 사용 65,535자 까지 저장 가능    최대 65,535자까지 저장가능
MEDIUMTEXT : TEXT보다 큰 텍스트 를 저장                         최대16MB
LONGTEXT   : TEXT, MEDIUMTEXT보다 큰 택스트를 저장              최대 4GB 
...
SET : 여러 값을 가질수 있는 집합을 정의하는 자료형 (다수 선택 가능)
