...
EX)
    INSERT 작업 완료! -------------------------------------> DB에 저장(X)
    INSERT 작업 완료! -----> 트랜잭션에 추가---> COMMIT-----> DB에 저장(O)
    ...
    2) ROLLBACK :   최종저장 취소
                    ...
                    마지막 커밋 상태로 돌아감(== 작업재용 삭제)

    3) SAVEPOINT :  임시저장
                    메모리 버퍼(트랜잭션)에 저장 지접을 설정해서 ROLLBACK 수행 시 전체 작업을 삭제하는 것이 아닌
                    저장지점까지만 일부 ROLLBACK
                ...
                ROLLBACK TO 임시저장이름1; --> 임시저장이름1번 까지 수정 삭제 삽입한 데이터삭제하고 복구
...
Atomicity(원자성) : 트랜잭션에 포함된 작업이 모두 성공하거나 실패해야함
		중간에 일부만 실행 X
		예) 은행 송금시, 계좌 A에서 돈이 빠져나갔는데
	 	     계좌 B에 입금이 안되면 안되기 때문에 둘 중 하나만 실행할 수 없다.

Consistency(일관성): 트랜잭션이 실행되기 전과 후의 데이터 상태가 일관되어야 함
		예) 은행 C계좌 총액이 10만원 이었다면, A,B나 다른 거래들이 일어났을 때도 10만원 이어야함
		     C가 계좌 송금 출금 같은 행위를 하지 않는 한

Isolation(격리성) : 여러 트랜잭션이 동시에 실행될 때, 서로의 작업에 영향을 주면 안됨
	            예) 두 사람이 동시에 같은 계좌에서 돈을 빼려 하면 충돌이 일어날 수 있으니
		   하나씩 순서대로 처리
