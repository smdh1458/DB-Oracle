...
PRIMARY KEY(기본 키)
    ...
    - 기본 키로 설정된 컬럼은 자동으로 UNIQUE와 NOT NULL 제약 조건을 가짐
      ...
      14번이거나 13번 이후 번호가 되어야지 회원가입 순 번이 13이거나 빈 값으면 안된다는 의미 
...
NOT NULL
    ...
    ->
    유저명   : 회원가입시 유저가 유저명 작성하지 않으면 회원가입이 X
...
UNIQUE 
    ...
    이메일이 1번 회원과 1번 이외 다른 회원과 일치하는 이메일 동일한 이메일이 없어야함
    마찬가지로 주민번호, 회원번호 또한 다른 유저와 일치하는 번호값이 없어야함

    1   kim@google.co.kr    000101-0101010
        kim@google.co.kr    000101-0101010 중복이므로 unique 하지 못함!             -> DB 저장 안됨
        kim@google.co.kr    000101-0202020 이메일이 중복이므로 unique 하지 못함!    -> DB 저장 안됨
        lee@google.co.kr    000101-0101010 주민번호가 중복이므로 unique 하지 못함!  -> DB 저장 안됨
    2   lee@google.co.kr    000101-0202020 주민번호와 이메일 모두 일치하는 회원이 없기 때문에 저장 가능
...
FOREIGN KEY
    ...
    customers_id INT
    FOREIGN KEY (customers_id) REFERENCES  customers(customer_id) ON DELETE CASCADE
    FOREIGN KEY : 다른 테이블에 작성된 키를
    현재 테이블에 위치한 특정 컬럼 id값에서
    REFERENCES  : 참조할 것이다.
    다른 테이블 이름(다른테이블에 작성된 id값)
