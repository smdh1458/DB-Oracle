...
2) VIEW를 이용한 DML(INSERT, UPDATE, DELETE)이 사용 가능한 경우도 있지만
...
5) WITH READ ONLY 옵션 : --ORACLE에서만 가능
            뷰에 대하 SELECT 만 가능하도록 지정
...
* B*트리 하나의 노드에 많은 수의 정보를 가질 수 있음 / 검색 속도 향상
    ...
    ** INDEX의 장점 **
    ...
    - 조회 시 테이블의 전체 내용을 확인하며 조회하는 것이 아닌 인덱스가 지정된 컬럼만을 이용해서 조회하기
      때문에 시스템의 부하가 낮아짐
    ...
    ** INDEX의 단점 **
    ...
    - 인덱스도 하나의 객체이다 보니 별도 저장공간이 필요(메모리 소모)
...
DROP INDEX 인덱스명;
    ...
    ** 인덱스가 자동으로 생성되는 경우 ** 
    ...
    -- 인덱스 사용 방법! -> WHERE 절에 INDEX가 저장된 컬럼 언급하기
