-- DATABASE 접속 KHTSHPPING 데이터 베이스 접속 후 PRODUCT 테이블 생성하고 생성 확인하기
...
-- KHTUSER 데이터 베이스에서 --
-- USER TABLE에 있는 홍길동의 이메일주소와 전화번호 조회
SELECT USERNAME,EMAIL, PHONE
FROM USER
WHERE USERNAME = '홍길동';

-- KHTSHOPPING 데이터 베이스 --
-- `ORDER` 테이블 주분상태가 PENDING 인 주문 수를 조회 오직 주문수
USE KHTSHOPPING;
...
-- KHTUSER 데이터베이스에서 LOGIN 활동을 한 사용자들의 이름과 세부활동 정보 확인
-- JOIN ON
USE KHTUSER;
...
-- UPDATE 문을 SQL에서 직접적으로 실행할 때 문제가 되는 것을 방지하기 위해 SAFE 모드 생성 (MYSQL에서만 그럼)
-- 테이블을 삭제할 때도 마찬가지로 SAFE 모드이므로 SAFE모드를 해지 후 실행 가능
...
-- 테이블 자체 수정 tables
-- STATUS 안에는 AVAILABLE, OCCUPIED 삽입 OK
-- enum('AVAILABLE','OCCUPIED')
-- CREATE TABLE SET - CHECKBOX 
-- CREATE TABE ENUM - RADIO

-- 테이블자체 컬럼에 존재하는 규칙 수정
-- ALTER TABLE 수정할 테이블명칭 MODIFY COLUMN 컬럼명 컬럼명의 규칙
ALTER TABLE TABLES MODIFY COLUMN STATUS ENUM('AVAILABLE','OCCUPIED','사용가능','사용중');

UPDATE TABLES SET STATUS = '사용가능'
WHERE STATUS = 'AVAILABLE';
...
ALTER TABLE TABLES MODIFY COLUMN STATUS ENUM('사용가능','사용중');
COMMIT;
SELECT *
FROM TABLES;
...
ALTER TABLE CAFE_ORDER MODIFY COLUMN STATUS ENUM('PENDING','PREPARING','COMPLETED','CANCELLED','주문 준비 중','주문 완료','주문 대기 중','주문 취소');

UPDATE CAFE_ORDER SET STATUS = '주문 준비 중'
WHERE STATUS = 'PREPARING';
...
ALTER TABLE CAFE_ORDER MODIFY COLUMN STATUS ENUM('주문 준비 중','주문 완료','주문 대기 중','주문 취소');
COMMIT;
