/*
USE DATABASE; 명칭은
한 회사에서 여러 서비스를 제공하고 각 서비스에 따른 데이터베이스에 접속할 때 사용하는 기능

예를 들어 KEKEO 회사가 MALON, DEUM, KEKEKOTALK 데이터를 관리할 때
...
-- 유저가 보고자하는 데이터베이스가 존재하지 않을 때 발생하는 Error Code: 1046.
...
-- SELECT INSERT UPDATE DELETE와 같은 QUERY 작업을 진행할 때
...
/*
--  USER_ID 기본키 자동 증가
-- USERNAME 문자열 빈 값 못들어가게 설정 (50BYTE)
--    EMAIL 문자열 빈 값 못들어가게 설정 (100BYTE)

	BIRTHDATE DATE 
    ACCOUNT_BALANCE DECIMAL(10,2) 정수 10글자, 소수점 2자리까지만 설정
    GENDER	ENUM('MAIL','FEMAIL')
    HOBBIES SET('READING','SPROTS','RUNNING')
...
-- khtdb에 userprofile 이라는 테이블이 존재하지 않으므로 CREATE TABLE USERPROFILE 을 이용해서
...
-- 테이블 삭제 * 주의 테이블 삭제 함부로 하지말것 반드시 혼자하더라도 상의 후 삭제할 것
