...
-- PRODUCT
-- CONCAT 이용해서 제품명: 울트라 게이밍 키보드, 가격 89000원으로 출력
USE KHTSHOPPING;
SELECT * FROM PRODUCT;

SELECT CONCAT('제품명:',NAME,',',PRICE)
FROM PRODUCT
WHERE PRODUCT_ID = 3;
-- SELECT CONCAT('제품명: ',NAME,', 가격 ',FLOOR(PRICE))
...
-- REPLACE 사용 방식 JS, JAVA, SQL등 거의 모든 언어에서 동일
...
O.TOTAL_AMOUNT, OI.PRODUCT_ID, OI.QUANTITY, OI.PRICE
FROM `ORDER` O
/*INNER*/JOIN ORDER_ITEM OI
ON O.ORDER_ID = OI.ORDER_ID;
-- 조건이 맞는 행만 가져옴

SELECT P.NAME,O.ORDER_ID, O.USER_ID, O.STATUS, 
		O.TOTAL_AMOUNT, OI.PRODUCT_ID, OI.QUANTITY, OI.PRICE
FROM `ORDER` O
JOIN ORDER_ITEM OI
ON O.ORDER_ID = OI.ORDER_ID
...
FROM `ORDER` O
LEFT JOIN REVIEW R
ON O.USER_ID = R.USER_ID; -- 모든 제품의 리뷰가 존재하기 때문에 NULL값이 생길 일이 없음

-- REVIEW 테이블과 ORDER 테이블을 RIGHT JOIN으로 연결해서 모든 리뷰가 포함되도록 하고,
-- 주문 정보가 없는 경우에는 NULL 표기되게 하기
